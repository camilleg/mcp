...
public:
	...
	inline array() : m(0), n(0), k(0), v(NULL), alias(false) {}
	inline array( int _m, int _n = 1, int _k = 1) 
	 : m(_m), n(_n), k(_k), alias(false) { if( m*n*k) v = new T[m*n*k]; else v = NULL;}
	inline array( T *p, int M, int N = 1, int K = 1) : m(M), n(N), k(K), v(p), alias(true) {}
	inline array( const array<T> &b) : m(b.m), n(b.n), k(b.k), v(NULL), alias(false)
	{
		try{
			v = new T[m*n*k];
		}
